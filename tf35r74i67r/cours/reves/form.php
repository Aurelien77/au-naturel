...
$requete = $bdd->prepare("SELECT * FROM tb_image2 WHERE id = :id ");
...
tb_image2 SET nom = ?, info = ?, commentaire = ?, date = ?, categorie = ? WHERE id = ?");
...
$requete = $bdd->prepare("INSERT INTO tb_image2(nom, info, commentaire, date, categorie) VALUES(?,?,?,?,?)");
...
$requete = $bdd->prepare("SELECT * FROM tb_image2 WHERE categorie = :categorie ");
$requete->execute(array(
...
$bdd = new PDO('mysql:host=aunatupma1830400.mysql.db;dbname=aunatupma1830400', 'aunatupma1830400', 'Neptune240984');
$requete = $bdd->query("SELECT categorie FROM tb_image2 GROUP BY categorie" );
